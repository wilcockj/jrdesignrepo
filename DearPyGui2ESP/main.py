...
layout = [ [sg.Button('Animation 1', button_color=('#ff5e00'), 
            ...
            sg.ColorChooserButton('Color Picker', button_color=('#000000'), 
                target='set_line_color', key='set_line_color_chooser', font = font, size = (10,1.5)), 
...
while True:
    ...
    if event == 'set_line_color': # if color picker is clicked, color picker window opens
        if values[event] == 'None': # if no color, default color is black
            values[event] = '#000000' 
    ...
    if event == 'audio_color': # if audio button is clicked
       if data['audio'] == 0: # if not selected, update json and change color
           data['audio'] = 1
    ...
    if event == 'Send': # if send button is clicked
        data['message'] = values[0] # update message from message window
